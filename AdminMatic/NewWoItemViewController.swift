...
class NewWoItemViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, NoInternetDelegate{
    ...
    @objc func submit(){
        ...
        parameters = ["woID": self.woID!,"itemID": selectedID,"estQty": estQtyString,"price": priceString,"chargeID": self.charge,"createdBy": self.appDelegate.defaults.string(forKey: loggedInKeys.loggedInId), "sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!] as! [String : String]
        
        print("parameters : \(parameters)")
                
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/new/workOrderItem.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
                    .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
                    .responseString { response in
