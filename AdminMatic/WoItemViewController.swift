...
class WoItemViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, WoItemDelegate, AttachmentDelegate, EditLeadDelegate, NoInternetDelegate{
    ...
    init(_woID:String,_woItem:WoItem2,_empsOnWo:[Employee2],_woStatus:String, _allowImages:String){
        super.init(nibName:nil,bundle:nil)
        self.woID = _woID
        self.woItem = _woItem
        self.empsOnWo = _empsOnWo
        self.newWoStatus = _woStatus
        self.allowImages = _allowImages
    ...
    func getWoItem(woItemID:String){
        ...
        if CheckInternet.Connection() != true{
            ...
            layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/workOrderItem.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.row == self.woItem.tasks!.count){
            ...
            if(self.woItem.tasks![indexPath.row].images!.count > 0){
                ...
                self.woItem.tasks![indexPath.row].images![0].setImagePaths(_thumbBase: self.woItem.tasks![indexPath.row].thumbBase!, _mediumBase: self.woItem.tasks![indexPath.row].mediumBase!, _rawBase: self.woItem.tasks![indexPath.row].rawBase!)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(indexPath.row == self.woItem.tasks!.count){
            ...
            imageUploadPrepViewController = ImageUploadPrepViewController(_imageType: "Task", _taskID: self.woItem.tasks![indexPath.row].ID, _customerID: self.customerID, _images: self.woItem.tasks![indexPath.row].images!,_allowImages: self.allowImages)
    ...
    func setTaskStatus(_ID:String,_status:String,_row:Int)->String{
        ...
        print("parameters = \(parameters)")
        self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/taskStatus.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
            response in
    ...
    func showImageActionSheet(_ID:String, _row:Int){
        ...
        actionSheet.addAction(UIAlertAction(title: "Add Task Image(s)", style: UIAlertAction.Style.default, handler: { (alert:UIAlertAction!) -> Void in
            let imageUploadPrepViewController:ImageUploadPrepViewController = ImageUploadPrepViewController(_imageType: "Task", _taskID: _ID, _customerID: self.customerID, _images: [], _allowImages:self.allowImages)
    ...
    func addTask(){
        ...
        let imageUploadPrepViewController:ImageUploadPrepViewController = ImageUploadPrepViewController(_imageType: "Task", _taskID: "0", _customerID: self.customerID, _images: [], _allowImages:self.allowImages)
    ...
    func getLead() {
        ...
        parameters = ["leadID": self.lead!.ID, "sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
        print("parameters = \(parameters)")
