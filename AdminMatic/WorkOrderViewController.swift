...
class WorkOrderViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate, UIGestureRecognizerDelegate, WoDelegate, StackDelegate, EditLeadDelegate, NoInternetDelegate {
    ...
    func refreshWo(_refeshWoID _refreshWoID:String, _newWoStatus:String){
        ...
        if(self.workOrder.status != _newWoStatus && _newWoStatus != "na"){
            ...
            let okAction = UIAlertAction(title: "Yes", style: UIAlertAction.Style.default) {
                ...
                self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/workOrderStatus.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                    response in
    ...
    func getWorkOrder() {
        ...
        //let woURL = "https://www.adminmatic.com/cp/app/functions/get/workOrder.php?woID=\(self.workOrderID!)"
      ...
      // print(woURL)
       ...
       // Alamofire.request(woURL).responseJSON { response in
        ...
        var parameters:[String:String]
        parameters = ["woID":self.workOrderID!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!,"companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
                            //print("parameters = \(parameters)")
                            
                            //print("remove signature from array")
                        
            self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/workOrder.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                                response in
                                //print(response.request ?? "")  // original URL request
                                //print(response.result)   // result of response serialization
            ...
            do{
                ...
                print("workOrder.allowImages = \(String(describing: self.workOrder.allowImages))")
 ...
 */
    ...
    @objc func handleStatusChange(){
        ...
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/workOrderStatus.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
            response in
            print(response.request ?? "")  // original URL request
            //print(response.response ?? "") // URL response
            //print(response.data ?? "")     // server data
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(indexPath.row == self.workOrder.woItems!.count){
            ...
            if(currentCell.woItem != nil && currentCell.woItem.ID != ""){
                self.woItemViewController = WoItemViewController(_woID: self.workOrder.ID, _woItem: currentCell.woItem, _empsOnWo: self.workOrder.emps!, _woStatus: self.workOrder.status, _allowImages: self.workOrder.allowImages! )
    ...
    func addItem(){
        ...
        newWoItemViewController.loadItemList()
