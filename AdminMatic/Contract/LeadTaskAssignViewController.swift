...
class LeadTaskAssignViewController: UIViewController,UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, AttachmentDelegate, LeadTaskDelegate, NoInternetDelegate{
    ...
    */
    ...
    func getWoItems() {
        ...
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/leadCustomerWoSearch.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
            .responseString { response in
    ...
    func getContractItems() {
        ...
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/leadCustomerContractSearch.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
            .responseString { response in
                //print("item response = \(response)")
            }
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let notNeeded = UITableViewRowAction(style: .normal, title: "N/A") { action, index in
            ...
            //print("parameters = \(parameters)")
            self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/changeField.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                response in
            ...
            if(self.lead.createdBy == self.appDelegate.loggedInEmployee?.ID){
                if(self.tasksArray[indexPath.row].ID != "0"){
                    ...
                    let okAction = UIAlertAction(title: "Yes", style: UIAlertAction.Style.default) {
                        ...
                        //print("parameters = \(parameters)")
                        self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/delete/leadTask.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                            response in
                            //print(response.request ?? "")  // original URL request
                            //print(response.result)   // result of response serialization
       ...
       // print("handleItemSelect selectedrow = \(schedulePicker.selectedRow(inComponent: 0))")
        ...
        func callAlamoFire(){
            if(tasksToLogJSON.count > 0){
                ...
                layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/leadTaskToItem.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
                    .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
                    .responseString { response in
