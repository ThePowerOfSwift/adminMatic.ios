...
class NewEditContractItemViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate,  UIPickerViewDelegate, UIPickerViewDataSource, NoInternetDelegate{
       ...
       // print("self.estQtyLbl.text = \(self.estQtyLbl.text)")
        ...
        if editMode == false{
           ...
           // //print("sort = \(itemCount)")
            ...
            parameters = ["contractItemID": "0","contractID":self.contract.ID,"itemID": self.contractItem.itemID, "type":self.contractItem.type!, "chargeType": self.contractItem.chargeType, "qty": self.contractItem.qty, "price": self.contractItem.price!, "total":self.contractItem.total!, "name":self.contractItem.name,"taxCode":self.contractItem.taxCode!,"subcontractor":self.contractItem.subcontractor!,"hideUnits":self.contractItem.hideUnits!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
        }else{
            ...
            parameters = ["contractItemID": self.contractItem.ID,"contractID":self.contract.ID,"itemID": self.contractItem.itemID, "type":self.contractItem.type!, "chargeType": self.contractItem.chargeType, "qty": self.contractItem.qty, "price": self.contractItem.price!, "total":self.contractItem.total!, "name":self.contractItem.name,"taxCode":self.contractItem.taxCode!,"subcontractor":self.contractItem.subcontractor!,"hideUnits":self.contractItem.hideUnits!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
        }
        
        
        print("parameters : \(parameters)")
        
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/contractItem.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
            .responseString { response in
