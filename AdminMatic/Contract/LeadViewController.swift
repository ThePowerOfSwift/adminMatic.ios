...
class LeadViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate, UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource, AttachmentDelegate, EditLeadDelegate, StackDelegate, LeadTaskDelegate, NoInternetDelegate {
    ...
    func getLead() {
        ...
        parameters = ["leadID": self.lead.ID,"companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!]
        print("parameters = \(parameters)")
        
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/lead.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            ...
            .responseJSON(){
                ...
                do{
                    ...
                    print("lead.allowImages = \(String(describing: self.lead.allowImages))")
    ...
    @objc func handleStatusChange(){
        ...
        if self.layoutVars.grantAccess(_level: 1,_view: self) {
            ...
            //print("parameters = \(parameters)")
            layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/leadStatus.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                response in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if(indexPath.row == self.lead.tasksArray!.count){
            ...
            imageUploadPrepViewController = ImageUploadPrepViewController(_imageType: "Lead Task", _leadID: self.lead.ID, _leadTaskID: self.lead.tasksArray![indexPath.row].ID, _customerID: self.lead.customerID!, _images: self.lead.tasksArray![indexPath.row].images!, _allowImages:self.lead.allowImages!)
        ...
        if (editingStyle == UITableViewCell.EditingStyle.delete) {
            ...
            if(lead.createdBy == appDelegate.loggedInEmployee?.ID){
                if(lead.tasksArray![indexPath.row].ID != "0"){
                    ...
                    //print("parameters = \(parameters)")
                    layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/delete/leadTask.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                        response in
                        //print(response.request ?? "")  // original URL request
                        //print(response.result)   // result of response serialization
    ...
    @objc func addTask(){
        ...
        let imageUploadPrepViewController:ImageUploadPrepViewController = ImageUploadPrepViewController(_imageType: "Lead Task", _leadID: self.lead.ID, _leadTaskID: "0", _customerID: self.lead.customerID!, _images: [], _allowImages:self.lead.allowImages!)
    ...
    func updateLead(_lead: Lead2, _newStatusValue:String){
        ...
        if(self.lead.statusID != _newStatusValue && _newStatusValue != "na"){
            ...
            let okAction = UIAlertAction(title: "Yes", style: UIAlertAction.Style.default) {
                ...
                //print("parameters = \(parameters)")
                self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/leadStatus.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                    response in
                    //print(response.request ?? "")  // original URL request
                    //print(response.result)   // result of response serialization
                    self.editsMade = true
