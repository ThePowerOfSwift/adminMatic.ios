...
protocol EditEquipmentDelegate{
    func updateEquipment(_equipment:Equipment2)
    func updateServiceList()
}


class EquipmentViewController: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UITableViewDelegate, UITableViewDataSource, EditEquipmentDelegate, NoInternetDelegate{
    ...
    var serviceCurrentArray:[EquipmentService2] = []
    var equipmentHistoryJSON: JSON!
    var serviceHistoryArray:[EquipmentService2] = []
    ...
    func getEquipmentServiceInfo(){
        ...
        parameters = ["equipmentID":equipment.ID,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
        print("parameters = \(parameters)")
        self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/equipment.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
    ...
    @objc func handleStatusChange(){
        ...
        if self.statusValueToUpdate != nil{
            ...
            layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/equipmentStatus.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                response in
