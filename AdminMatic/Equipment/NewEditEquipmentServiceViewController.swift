...
class NewEditEquipmentServiceViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate, NoInternetDelegate {
    ...
    //init for edit
    init(_equipmentService:EquipmentService2){
        super.init(nibName:nil,bundle:nil)
    ...
    func layoutViews(){
        ...
        if(self.equipmentService == nil){
            ...
            self.equipmentService = EquipmentService2(_ID: "0", _name: "", _type: "0", _typeName: "One Time", _createdBy: "", _status: "0", _equipmentID: self.equipmentID)
        }else{
            title =  "Edit Service"
        }
    ...
    @objc func submit(){
        ...
        parameters = ["ID": self.equipmentService.ID, "addedBy": self.appDelegate.defaults.string(forKey: loggedInKeys.loggedInId)!, "name": self.equipmentService.name, "type": self.equipmentService.type,"frequency": self.equipmentService.frequency!, "instructions": self.equipmentService.instruction!, "equipmentID": self.equipmentService.equipmentID, "status": self.equipmentService.status, "nextValue": self.equipmentService.nextValue!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
        
        print("parameters = \(parameters)")
        
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/equipmentService.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
