...
class EquipmentFieldsListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, EquipmentFieldListDelegate, NoInternetDelegate{
    ...
    func getEquipmentFields(){
        ...
        parameters = [
            "sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!
        ]
    ...
    func deleteField(_ID:String){
        ...
        if CheckInternet.Connection() != true{
            ...
            print("parameters = \(parameters)")
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/delete/equipmentField.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
            .responseString { response in
                print("field delete response = \(response)")
            }
