...
class EquipmentListViewController: ViewControllerWithMenu, UITableViewDelegate, UITableViewDataSource, UISearchControllerDelegate, UISearchBarDelegate, UISearchDisplayDelegate, UISearchResultsUpdating, EquipmentListDelegate, NoInternetDelegate{
    ...
    var equipmentArray:[Equipment2] = []
    var equipmentSearchResults:[Equipment2] = []
    ...
    func getEquipmentList() {
        ...
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/equipmentList.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
    ...
    func filterSearchResults(){
        self.equipmentSearchResults = self.equipmentArray.filter({( aEquipment: Equipment2) -> Bool in
    ...
    func deActivateEquipment(_ID:String){
        ...
        if self.layoutVars.grantAccess(_level: 1,_view: self) {
            ...
            let okAction = UIAlertAction(title: "Yes", style: UIAlertAction.Style.default) {
                ...
                parameters = ["equipmentID": _ID,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
                print("parameters = \(parameters)")
                
                // Show Indicator
                self.indicator = SDevIndicator.generate(self.view)!
                
                self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/equipmentActive.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
                    .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
                    .responseString { response in
                        //print("equipment response = \(response)")
                    }
