...
class PayrollSummaryViewController: ViewControllerWithMenu, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource, NoInternetDelegate{
    ...
    func getPayroll(){
       ...
       // print("start date = \(startDate)")
        ...
        let parameters = ["endDate": endDateDB,"empID":self.empID,"companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!] as [String : String]
        
        print("parameters = \(parameters)")
        
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/payroll.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            ...
            //if self.payrollJSON["payroll"]["\(day)"].stringValue != nil{
                ...
                for i in 0 ..< payrollShiftCount {
                    ...
                    if self.payrollJSON["payroll"]["\(day)"][i]["dayType"].string! == "0" && self.payrollJSON["payroll"]["\(day)"][i]["startTime"].string != "No Time"{
                        ...
                        let payroll:Payroll2!
                    payroll = Payroll2(_ID: self.payrollJSON["payroll"]["\(day)"][i]["ID"].string!, _empID: self.payrollJSON["payroll"]["\(day)"][i]["empID"].string!, _lunch: self.payrollJSON["payroll"]["\(day)"][i]["lunch"].string!, _date: date, _total: self.payrollJSON["payroll"]["\(day)"][i]["total"].string!, _verified: self.payrollJSON["payroll"]["\(day)"][i]["verified"].string!, _appCreatedBy: self.payrollJSON["payroll"]["\(day)"][i]["appCreatedBy"].string!)
