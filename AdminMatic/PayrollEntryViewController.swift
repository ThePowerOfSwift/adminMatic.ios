...
class PayrollEntryViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource, PayrollDelegate, NoInternetDelegate{
    ...
    func getPayroll(){
        ...
        let parameters:[String:String]
            parameters = ["endDate": endDateDB,"empID":self.employee.ID,"companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!]
   ...
   @objc func displayPayrollSummary(){
    ...
    func submitPayroll(){
        ...
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/update/payroll.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
            .responseString { response in
                //print("payroll response = \(response)")
            }
