...
class ShiftsViewController: ViewControllerWithMenu, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource, NoInternetDelegate{
    ...
    func getShifts(){
        ...
        let parameters:[String:String]
            parameters = ["startDate":  startDateDB,"endDate": endDateDB,"empID":self.empID,"companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!,"sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!]
        
        //print("parameters = \(parameters)")
        
        layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/shifts.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
       ...
       // print("parse shiftsJSON: \(self.shiftsJSON)")
        ...
        //print("shift count = \(shiftsCount)")
            ...
            //print("day = \(day)")
                ...
                let shift:Shift2!
            shift = Shift2(_ID: self.shiftsJSON!["shifts"]["\(day)"]["ID"].string!, _empID: self.shiftsJSON!["shifts"]["\(day)"]["empID"].string!)
                    
                    shift.startTime = startTime
                    shift.stopTime = stopTime
                    shift.comment = self.shiftsJSON!["shifts"]["\(day)"]["comment"].string!
                    shift.qty = self.shiftsJSON!["shifts"]["\(day)"]["shiftQty"].string!
                    shift.status = self.shiftsJSON!["shifts"]["\(day)"]["status"].string!
                    
                    
                    //shift = Shift2(_ID: self.shiftsJSON!["shifts"]["\(day)"]["ID"].string!, _empID: self.shiftsJSON!["shifts"]["\(day)"]["empID"].string!, _startTime: startTime, _stopTime: stopTime, _status: self.shiftsJSON!["shifts"]["\(day)"]["status"].string!, _comment: self.shiftsJSON!["shifts"]["\(day)"]["comment"].string!, _qty: self.shiftsJSON!["shifts"]["\(day)"]["shiftQty"].string!)
                   // shift = Shift2(_ID: self.shiftsJSON!["shifts"]["\(day)"]["ID"].string!, _empID: <#T##String#>)
