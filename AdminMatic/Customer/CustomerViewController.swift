...
class CustomerViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, ScheduleDelegate, ImageViewDelegate, ImageLikeDelegate, LeadListDelegate, ContractListDelegate, InvoiceListDelegate, EditCustomerDelegate, NoInternetDelegate{
    ...
    var allowImages:String = "1"
    var allowImagesLbl:Label = Label()
    ...
    func getCustomerData(_id:String){
        ...
        print("get cust data with id: \(_id)")
        ...
        //Get lead list
               ...
               parameters = ["sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)! as AnyObject, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)! as AnyObject,"ID":self.customerID as AnyObject]
               ...
               self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/customer.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
                   ...
                   .responseJSON() {
                    ...
                    do{
                        ...
                        self.phone = "No Phone Found"
                        self.phoneName = ""
                        self.email = "No Email Found"
                        self.emailName = ""
                        self.jobSiteAddress = "No Job Site Found"
                        
                        self.customerName = self.customer.sysname
                               
                        self.allowImages = self.customer.allowImages!
                    ...
                    do {
                        ...
                        let json = response.data
                        let decoder = JSONDecoder()
                        let parsedData = try decoder.decode(Customer2.self, from: json!)
                        print("parsedData = \(parsedData)")
                        let customer = parsedData
                        ...
                        self.customerName = customer.sysname
                               
                        self.allowImages = customer.allowImages!
                               
                               //loop through contacts and put them in appropriate places
                        let contactCount:Int = customer.contacts!.count
                               //print("contactCount: \(contactCount)")
                               for i in 0 ..< contactCount {
                                
                                switch  customer.contacts![i].type! {
                                //phone
                                case "1":
                                    //print("case = phone")
                                     //print("phone = \(self.customerJSON["customer"]["contacts"][i]["value"].string!)")
                                    //print("phone = \(self.customerJSON["customer"]["contacts"][i]["value"].string!)")
                                    self.phone = customer.contacts![i].value!
                                    if customer.contacts![i].name != nil
                                        {
                                            self.phoneName = " (" + customer.contacts![i].name! + ")"
                                ...
                                case "2":
                                    //print("case = email")
                                        self.email = customer.contacts![i].value!
                                        if customer.contacts![i].name != nil
                                        {
                                            self.emailName = " (" + customer.contacts![i].name! + ")"
                                        }
                                    break
                                ...
                                case "4":
                                    ...
                                    self.jobSiteAddress = customer.contacts![i].fullAddress!
                                    self.lat = customer.contacts![i].lat! as NSString
                                    self.lng = customer.contacts![i].lng! as NSString
                                    //print(" lat \(self.lat)")
                        ...
                        if self.customerContactViewController != nil{
                            ...
                            self.customerContactViewController?.updateContactList(_contacts: self.customer.contacts!)
    ...
    func getLeads(_openNewLead:Bool){
        ...
        //Get lead list
        var parameters:[String:AnyObject]
        parameters = ["custID":self.customerID as AnyObject, "sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)! as AnyObject, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)! as AnyObject]
        //print("parameters = \(parameters)")
            ...
            .responseJSON() {
                ...
                do{
                    ...
                    for i in 0 ..< leadCount {
                        ...
                        self.customerLeadArray.append(leads.leads[i])
                    }
    ...
    func getContracts(_openNewContract :Bool){
       ...
       // print("parameters = \(parameters)")
        ...
        self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/contracts.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate()    // or, if you just want to check status codes, validate(statusCode: 200..<300)
            .responseString { response in
    ...
    func getCustomerSchedule(_id:String){
        ...
        var parameters:[String:String]
              parameters = ["empID":"","custID":_id,"active":"1", "sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)!, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)!]
                            //print("parameters = \(parameters)")
                            
                            //print("remove signature from array")
                        
              self.layoutVars.manager.request("https://www.adminmatic.com/cp/app/functions/get/workOrders.php",method: .post, parameters: parameters, encoding: URLEncoding.default, headers: nil).responseJSON() {
                                response in
 ...
 */
    ...
    func getImages() {
        ...
        if allowImages == "0"{
            print("customer does not want images")
        ...
        parameters = ["loginID": self.appDelegate.loggedInEmployee?.ID as AnyObject,"limit": "\(self.limit)" as AnyObject,"offset": "\(self.offset)" as AnyObject, "order":self.order as AnyObject, "customer": self.customerID as AnyObject, "sessionKey": self.appDelegate.defaults.string(forKey: loggedInKeys.sessionKey)! as AnyObject, "companyUnique": self.appDelegate.defaults.string(forKey: loggedInKeys.companyUnique)! as AnyObject]
    ...
    func layoutViews(){
        ...
        allowImagesLbl.text = "No Image Collection"
        allowImagesLbl.textColor = UIColor.red
        allowImagesLbl.textAlignment = NSTextAlignment.center
        allowImagesLbl.font = layoutVars.largeFont
        self.detailsView.addSubview(self.allowImagesLbl)
        allowImagesLbl.isHidden = true
        ...
        let customerDetailsViewsDictionary = [
            ...
            "view10":self.allowImagesLbl,
            "view11":self.addLeadBtn,
            "view12":self.addContractBtn,
            "view13":self.addWorkOrderBtn,
            "view14":addInvoiceBtn,
        ] as [String : Any]
        ...
        self.detailsView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|[view13(width)]", options: [], metrics: sizeVals, views: customerDetailsViewsDictionary))
         self.detailsView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|[view14(width)]", options: [], metrics: sizeVals, views: customerDetailsViewsDictionary))
        ...
        self.detailsView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[view1(40)]-[view10(40)]", options: [], metrics: sizeVals, views: customerDetailsViewsDictionary))
        ...
        self.detailsView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[view14(40)]|", options: [], metrics: sizeVals, views: customerDetailsViewsDictionary))
    ...
    @objc func showAllContacts(){
        self.customerContactViewController = CustomerContactViewController(_customerID: self.customerID, _contacts: self.customer.contacts!, _notes: self.customer.custNotes!)
    ...
    @objc func addWorkOrder(){
        ...
        let newWorkOrderViewController = NewEditWoViewController(_customer: self.customer)
    ...
    func switchTableView(_displayMode:String){
        ...
        case "IMAGES":
           ...
           if self.allowImages == "0"{
                allowImagesLbl.isHidden = false
                self.noImagesLbl.isHidden = true
                self.imageCollectionView?.isHidden = true
    ...
    func reloadData() {
        self.getCustomerData(_id: self.customerID)
       }
